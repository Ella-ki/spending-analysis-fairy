...
현대카드 XLS 이용명세서를 한 달에 한 번 업로드하면, 부부가 함께 이번 달 카드 소비를 빠르게 확인할 수 있는 모바일 우선 소비 분석 대시보드입니다.
...
- 현대카드 XLS 이용명세서 업로드
- 거래 내역 가져오기 및 정규화
- Supabase Storage에 원본 명세서 파일 보관
- 대시보드 지표
...
실제 로그인, DB 저장, 명세서 업로드까지 사용하려면 Supabase 계정이 필요합니다.
...
## 8. 현대카드 XLS 이용명세서 업로드

앱 하단의 `업로드` 메뉴에서 현대카드에서 내려받은 XLS 이용명세서를 올립니다.
...
- `쿠팡`을 `Shopping`으로 변경
- 다음 명세서 가져오기부터 `쿠팡`은 자동으로 `Shopping` 처리
